...
export const onDealStageChanged = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (after.contactId && (after.stage === DealStage.Won || after.stage === DealStage.Lost)) {
      const contactRef = db.collection('contacts').doc(after.contactId);
      const contactUpdate: Partial<{ lifecycleStage: string; leadScore: number }> = {
...
export const onDealUpdated = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const fieldsToTrack: (keyof Deal)[] = ['title', 'value', 'probability', 'expectedCloseDate', 'ownerId'];
