...
exports.onTicketUpdated =
	exports.onTicketStatusChanged =
	exports.onTicketCreated =
		void 0;
...
const SLA_HOURS = {
	...
	[models_1.TicketPriority.Medium]: 48,
	[models_1.TicketPriority.High]: 24,
	[models_1.TicketPriority.Critical]: 4,
};
/**
...
exports.onTicketCreated = functions.firestore
	...
	.onCreate(async (snap, context) => {
		...
		if (!ticket.slaDeadline) {
			const slaHours =
				SLA_HOURS[ticket.priority] || SLA_HOURS[models_1.TicketPriority.Medium];
		...
		if (!ticket.status) {
			updates.status = models_1.TicketStatus.Open;
		}
		...
		const createActivity = {
			type: models_2.ActivityType.Note,
			ticketId: context.params.ticketId,
		...
		const auditLog = {
			userId: "system",
			action: models_3.AuditAction.Create,
			collection: "tickets",
 ...
 */
exports.onTicketStatusChanged = functions.firestore
	.document("tickets/{ticketId}")
	.onUpdate(async (change, context) => {
		...
		const auditLog = {
			userId: "system",
			action: models_3.AuditAction.Update,
			collection: "tickets",
			documentId: context.params.ticketId,
			changes: {
...
async function getNextAssignee() {
	...
	const ticketsQuery = await firebase_1.db
		...
		.where("status", "in", [
			models_1.TicketStatus.Open,
			models_1.TicketStatus.InProgress,
		])
		.get();
