    ...
    <header className="sticky top-0 z-50 w-full bg-white border-b border-secondary/10">
      <div className="max-w-7xl mx-auto px-6 lg:px-10 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {isAuthenticated ? (
            <Link href="/client-access" className="bg-secondary text-white px-5 py-2.5 rounded-lg text-sm font-bold transition-all hover:bg-secondary/90">
              Mi Portal
            </Link>
          ) : (
            <button onClick={openAuthModal} className="bg-secondary text-white px-5 py-2.5 rounded-lg text-sm font-bold transition-all hover:bg-secondary/90">
              Acceso Clientes
            </button>
          )}
