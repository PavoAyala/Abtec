...
export default function AuthModal(): ReactElement | null {
  ...
  const getFriendlyErrorMessage = (errorObj: unknown): string => {
    ...
    if (code.includes('auth/popup-closed-by-user')) return 'El inicio de sesión con Google fue cancelado.';
    ...
    if (code.includes('auth/email-already-in-use')) return 'Ya existe una cuenta con este correo electrónico.';
    ...
    return (
      <div className="fixed inset-0 z-100 bg-black/50 backdrop-blur-sm flex items-center justify-center">
        <div className="bg-white rounded-2xl w-full max-w-sm p-8 text-center relative shadow-2xl">
          <button onClick={closeAuthModal} className="absolute right-4 top-4 text-gray-400 hover:text-gray-600 transition-colors">
          ...
          <button onClick={closeAuthModal} className="w-full bg-primary text-white py-3 rounded-xl font-bold hover:bg-primary/90 transition-colors">
            Continuar al sitio
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-100 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md p-8 relative shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <button onClick={closeAuthModal} className="absolute right-4 top-4 text-secondary/40 hover:text-secondary p-1 transition-colors">
          <span className="material-symbols-outlined">close</span>
        </button>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-secondary/60 text-sm mt-1">Gestiona tus proyectos y servicios</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {activeTab === 'register' && (
            <div className="flex gap-4">
              <div className="flex-1">
                <label htmlFor="first-name-input" className="block text-xs font-semibold text-secondary/80 mb-1 ml-1 uppercase tracking-wide">Nombre</label>
                <input
                  id="first-name-input"
                  type="text"
                  required
              ...
              <div className="flex-1">
                <label htmlFor="last-name-input" className="block text-xs font-semibold text-secondary/80 mb-1 ml-1 uppercase tracking-wide">Apellido</label>
                <input
                  id="last-name-input"
                  type="text"
                  required
          ...
          <div>
            <label htmlFor="email-input" className="block text-xs font-semibold text-secondary/80 mb-1 ml-1 uppercase tracking-wide">Correo Electrónico</label>
            <input
              id="email-input"
              type="email"
              required
          ...
          <div>
            <label htmlFor="password-input" className="block text-xs font-semibold text-secondary/80 mb-1 ml-1 uppercase tracking-wide">Contraseña</label>
            <input
              id="password-input"
              type="password"
              required
          ...
          {activeTab === 'register' && (
            <div>
              <label htmlFor="confirm-password-input" className="block text-xs font-semibold text-secondary/80 mb-1 ml-1 uppercase tracking-wide">Confirmar Contraseña</label>
              <input
          ...
          >
            { (() => {
               ...
               return activeTab === 'login' ? 'Ingresar a mi cuenta' : 'Completar Registro';
            })() }
          </button>
        </form>
        ...
        <div className="my-6 flex items-center before:flex-1 before:border-t before:border-gray-200 after:flex-1 after:border-t after:border-gray-200">
          <span className="mx-4 text-sm text-gray-400 font-medium">o continúa con</span>
        </div>
